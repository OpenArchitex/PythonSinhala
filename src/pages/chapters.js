...
const sortedVideosForChapter = (videos, chapter_number) =>
  videos.filter(video => video.node.chapter_number === chapter_number).sort((a, b) => a.node.episode - b.node.episode);


const Chapters = ({ data }) => {
  const chapters = data?.allMongodbPythonsinhalaChapter.edges;
  const videos = data?.allMongodbPythonsinhalaVideo.edges;
  const firstVideo = videos.filter(video => video.node.chapter_number === 0 && video.node.episode === 0);
  const sortedChapters = chapters.toSorted((a, b) => a.node.chapter_number - b.node.chapter_number);
    ...
    <>
      ...
      <StyledContainer fluid>
        <Row>
            ...
            <Accordion defaultActiveKey="0">
              {sortedChapters.map(({node}) => (
                <Accordion.Item eventKey={node.chapter_number}>
                  ...
                  <Accordion.Body>
                    <ListGroup>
                      {sortedVideosForChapter(videos, node.chapter_number).map(video => <ListGroup.Item><Container onClick={() => setSelectedVideo(video)}><Button variant="light">{video.node.name}</Button></Container></ListGroup.Item>)}
                    </ListGroup>
                  </Accordion.Body>
                </Accordion.Item>
                ))}
          ...
          <Col md={9}><IFrameContainer fluid><ResponsiveIFrame title="Current Video"
            src={`${selectedVideo.node.url}?modestbranding=1`}
            allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture"
                              allowFullScreen></ResponsiveIFrame></IFrameContainer></Col>
        </Row>
      </StyledContainer>
      <Footer />
    </>
